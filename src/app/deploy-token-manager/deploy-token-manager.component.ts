...
@Component({
  ...
  template: `
    ...
    /></label>
    <label>Token 2 Address <input type="text" #address2 value="0xAF37342e003E90DCa134A3dF76C52aeEFC4bA39d"/></label>
...
export class DeployTokenManagerComponent {
  ...
  async create(
    ...
    const salt = ethers.utils.hexZeroPad(
      ethers.utils.hexlify(Math.round(31340)),
      32
    );
    ...
    try {
      ...
      localStorage['tokenManager'] = JSON.stringify({salt: salt,token:address1, id:TID, tokenManager: tokenManagerAddress});
      ...
      console.log("creating remote token manager with:",salt,chain2,tmType,this.params,ethers.utils.parseEther("0.1"));
